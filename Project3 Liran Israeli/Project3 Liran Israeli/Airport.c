...
{
	const Airport* p1 = (const Airport*)pPort1;
	const Airport* p2 = (const Airport*)pPort2;
...
int readAirportFromFile(Airport* pAirport, FILE* fp) {

	char temp[CHAR_MAX];

	myGetsFromFile(temp, CHAR_MAX, fp);
	pAirport->name = (char*)malloc(sizeof(char)*(strlen(temp)+1));
	if (!pAirport->name)
		return 0;
	strcpy(pAirport->name, temp);

	myGetsFromFile(temp, CHAR_MAX, fp);
	pAirport->country = (char*)malloc(sizeof(char)*(strlen(temp) + 1));
	if (!pAirport->country)
		return 0;
	strcpy(pAirport->country, temp);

	fscanf(fp, "%s", &pAirport->code);
